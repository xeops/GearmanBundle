...
class GearmanClient extends AbstractGearmanService
{
	...
	protected function assignServers(\GearmanClient $gearmanClient)
	{
		...
		$succeed = 0;
		if (isset($this->settings['distribute_tasks']) && $this->settings['distribute_tasks'] === true)
		{
			shuffle($servers);
		}
		else
		...
		{
			if (@$gearmanClient->addServer($server['host'], $server['port']))
			{
				$succeed++;
			}
		}
		if (!$succeed)
		{
			throw new ServerConnectionException('Client was unable to connect to any server.');
		}
	...
	{
		return array_filter(
			empty($this->servers) ? $this->defaultServers : $this->servers,
			function ($server)
			{
				return $this->isServerAvailable($server['host'], $server['port']) && $this->isServerIsAlive($server['host'], $server['port']);
			}
		);
	}

	private function isServerIsAlive($host, $port)
	{
		$temporaryClient = new \GearmanClient();
		return $temporaryClient->addServer($host, $port) && @$temporaryClient->ping(0);
	}
	...
	public function getJobStatus($idJob)
	{
		...
		return new JobStatus([$known, $isRunning]);
	}
